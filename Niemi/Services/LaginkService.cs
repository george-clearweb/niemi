...
public class LaginkService : ILaginkService
{
    ...
    public async Task<IEnumerable<LaginkHd>> GetInvoicedLaginkDataAsync(DateTime fromDate, DateTime toDate, int skip, int take)
    {
        var sw = System.Diagnostics.Stopwatch.StartNew();
        var results = new List<LaginkHd>();
        ...
        try
        {
            ...
            using var command = new FbCommand(@"
                ...
                FROM LAGINKHD h
                LEFT JOIN LAGINKRD r ON h.ORDERNR = r.ORDERNR
                WHERE h.INLEVDATUM >= @fromDate 
                  AND h.INLEVDATUM <= @toDate
                ORDER BY h.INLEVDATUM DESC, h.ORDERNR, r.RADNR", connection);
            ...
            _logger.LogInformation(
                "Executing invoiced LAGINK query with skip: {Skip}, take: {Take}, fromDate: {FromDate}, toDate: {ToDate}", 
            ...
            sw.Stop();
            _logger.LogInformation(
                "Invoiced LAGINK query completed in {ElapsedMs}ms. Found {HeaderCount} invoiced headers with their details", 
                sw.ElapsedMilliseconds, results.Count);
