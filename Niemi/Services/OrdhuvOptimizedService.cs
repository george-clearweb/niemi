...
namespace Niemi.Services;
    ...
    public async Task<IEnumerable<OrdhuvDto>> GetOrdersByLicensePlatesAsync(List<LicensePlateItem> licensePlates, string? environment = null, string[]? environments = null, string? orhStat = null, string? customerType = null)
    {
        var sw = System.Diagnostics.Stopwatch.StartNew();
        var allResults = new List<OrdhuvDto>();
        ...
        try
        {
            ...
            // Query all environments in parallel
            var tasks = targetEnvironments.Select(env => QueryEnvironmentByLicensePlatesAsync(env, licensePlates, orhStat, customerType));
            var environmentResults = await Task.WhenAll(tasks);
    ...
    /// </summary>
    private async Task<List<OrdhuvDto>> QueryEnvironmentByLicensePlatesAsync(string environment, List<LicensePlateItem> licensePlates, string? orhStat = null, string? customerType = null)
    {
        var results = new List<OrdhuvDto>();
        ...
        try
        {
            ...
            _logger.LogInformation("License plates SQL Parameters: licensePlates={LicensePlates}, orhStat={OrhStat}", 
                string.Join(",", licensePlates.Select(lp => lp.Licenseplate)), orhStat);
